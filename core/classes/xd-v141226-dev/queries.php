...
namespace xd_v141226_dev {
    ...
    class queries extends framework
    {
        ...
        /**
         * @var array Show posts associated with certain taxonomy
         */
        protected $tax_query = array(
            'relation' => 'OR', // The logical relationship between each inner taxonomy array when there is more than one. Possible values are 'AND', 'OR'. Do not use with a single inner taxonomy array
            array(
                'taxonomy' => '', // (string) - Taxonomy.
                'field' => 'term_id',// (string) - Select taxonomy term by ('term_id'(default), 'name' or 'slug')
                ...
                'operator' => 'IN' // (string) - Operator to test. Possible values are 'IN'(default), 'NOT IN', 'AND'.
            ),
        );
        /**
        ...
        /**
         * @var array Built in taxonomies slugs ('category', 'post_tag', 'link_category', 'nav_menu')
         */
        public $_builtInTaxonomiesSlugs = array('category', 'post_tag', 'link_category', 'nav_menu');
        /**
        ...
        protected function query_check_arg_types()
        {
            if ($this->_forceValidValues) {
                call_user_func_array(array($this, 'check_arg_types'), func_get_args());
            }
        }
    }
}